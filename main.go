...
func argsHandler() (string, []string) {
...
func startListeners(ports []string, host string) {
	// _ index of ports list declared but unused
	for _, port := range ports {
		_, err := connectToPort(host, port, "tcp")
		if err != nil {
			continue
		}
	}
...
func main() {
	host, ports := argsHandler()

	startListeners(ports, host)
}
